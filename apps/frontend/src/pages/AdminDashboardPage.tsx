...
const AdminDashboardPage = () => {
  ...
  const confirmRejection = () => {
    if (pendingRejectionId) {
      handleDeclineConsultant(pendingRejectionId);
    }
  };
                          ...
                          <TableCell>
                            <Box sx={{ display: 'flex', gap: 1 }}>
                              ...
                              {!consultant.isBanned && (
                                consultant.isVerified ? (
                                  <IconButton
                                    onClick={() => openRejectionDialog(consultant.id)}
                                    sx={{ 
                                      color: '#ff9800', 
            ...
            <>
              ...
              {selectedConsultant.status === 'pending' && (
                <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
                  <Button
                    onClick={() => openRejectionDialog(selectedConsultant.id)}
          ...
          {selectedWithdrawal && (
            <>
              <Box sx={{ p: 2, background: 'rgba(13, 180, 188, 0.05)', borderRadius: 2, mb: 3 }}>
                ...
                {selectedWithdrawal.withdrawalMethod && (
                  <Box sx={{ mt: 2, p: 2, bgcolor: '#f8f9fa', borderRadius: 1, border: '1px solid #e0e0e0' }}>
                    <Typography sx={{ fontSize: '0.9rem', fontWeight: 700, color: '#0db4bc', mb: 1.5 }}>
                      💳 Payment Details
                    </Typography>
                    {selectedWithdrawal.withdrawalMethod.type === 'bank_transfer' && (
                      <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1.5 }}>
                        <Box>
                          <Typography sx={{ fontSize: '0.75rem', color: '#888', fontWeight: 600 }}>Account Holder</Typography>
                          <Typography sx={{ fontSize: '0.9rem', fontWeight: 600 }}>{selectedWithdrawal.withdrawalMethod.accountHolderName || 'N/A'}</Typography>
                        </Box>
                        <Box>
                          <Typography sx={{ fontSize: '0.75rem', color: '#888', fontWeight: 600 }}>Account Number</Typography>
                          <Typography sx={{ fontSize: '0.9rem', fontWeight: 600 }}>{selectedWithdrawal.withdrawalMethod.accountNumber || 'N/A'}</Typography>
                        </Box>
                        <Box>
                          <Typography sx={{ fontSize: '0.75rem', color: '#888', fontWeight: 600 }}>Bank Name</Typography>
                          <Typography sx={{ fontSize: '0.9rem', fontWeight: 600 }}>{selectedWithdrawal.withdrawalMethod.bankName || 'N/A'}</Typography>
                        </Box>
                        <Box>
                          <Typography sx={{ fontSize: '0.75rem', color: '#888', fontWeight: 600 }}>SWIFT Code</Typography>
                          <Typography sx={{ fontSize: '0.9rem', fontWeight: 600 }}>{selectedWithdrawal.withdrawalMethod.swiftCode || 'N/A'}</Typography>
                        </Box>
                      </Box>
                    )}
                    {selectedWithdrawal.withdrawalMethod.type === 'paypal' && (
                      <Box>
                        <Typography sx={{ fontSize: '0.75rem', color: '#888', fontWeight: 600 }}>PayPal Email</Typography>
                        <Typography sx={{ fontSize: '0.9rem', fontWeight: 600 }}>{selectedWithdrawal.withdrawalMethod.email || 'N/A'}</Typography>
                      </Box>
                    )}
                    {selectedWithdrawal.withdrawalMethod.type === 'crypto' && (
                      <Box>
                        <Typography sx={{ fontSize: '0.75rem', color: '#888', fontWeight: 600 }}>Wallet Address</Typography>
                        <Typography sx={{ fontSize: '0.9rem', fontWeight: 600, wordBreak: 'break-all' }}>{selectedWithdrawal.withdrawalMethod.walletAddress || 'N/A'}</Typography>
                      </Box>
                    )}
                    {selectedWithdrawal.withdrawalMethod.type === 'stripe' && (
                      <Box>
                        <Typography sx={{ fontSize: '0.75rem', color: '#888', fontWeight: 600 }}>Account Email</Typography>
                        <Typography sx={{ fontSize: '0.9rem', fontWeight: 600 }}>{selectedWithdrawal.withdrawalMethod.email || 'N/A'}</Typography>
                      </Box>
                    )}
                  </Box>
      ...
      >
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
            ...
            <IconButton onClick={closeRejectionDialog} sx={{ color: '#666' }}>
              <FaTimes />
            </IconButton>
          </Box>

          <Typography sx={{ mb: 3, color: '#666' }}>
            Please provide a reason for declining this consultant's verification. This will help the consultant understand what needs to be improved.
          </Typography>
          ...
          <TextField
            ...
            placeholder="E.g., ID documents are unclear, missing required certifications, profile information incomplete..."
