...
export const declineConsultant = catchAsync(async (req: Request, res: Response) => {
  verifyAdmin(req);
  const result = await adminService.declineConsultant(req.params.consultantId!, req.body.reason);
  res.status(200).json({ success: true, data: result });
});
...
export const getConsultantDetailForAdmin = catchAsync(async (req: Request, res: Response) => {
  verifyAdmin(req);
  const consultantDetail = await adminService.getConsultantDetailForAdmin(req.params.consultantId!);
  res.status(200).json({ success: true, data: consultantDetail });
});

export const getVerifiedConsultants = catchAsync(async (req: Request, res: Response) => {
  verifyAdmin(req);
  const consultants = await adminService.getVerifiedConsultants();
  res.status(200).json({ success: true, data: consultants });
});
