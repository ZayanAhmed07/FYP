...
/**
 * Get Withdrawal Request Details with Account Information
 * Returns withdrawal details including account holder info and consultant details
 */
export const getWithdrawalDetail = catchAsync(async (req: Request, res: Response) => {
  const { withdrawalId } = req.params;
  ...
  const withdrawal = await Withdrawal.findById(withdrawalId)
    .populate('consultantId', 'name email phone profileImage')
    .lean();
  ...
  const consultant = await Consultant.findOne({ userId: withdrawal.consultantId })
    .select('title hourlyRate totalEarnings isVerified')
    .lean();
  ...
  res.status(200).json({
    ...
    data: {
      ...withdrawal,
      consultant,
      accountDetails: withdrawal.withdrawalMethod,
    },
  });
});
