...
const envSchema = Joi.object({
  ...
  CNIC_SERVICE_URL: Joi.string().uri().optional().allow(''),
})
  .unknown()
  .required();
...
export const env = {
  ...
  GEMINI_API_KEY: value.GEMINI_API_KEY as string,
  apiUrl: value.API_URL as string,
  CNIC_SERVICE_URL: (value.CNIC_SERVICE_URL as string) || 'http://127.0.0.1:5000',
};
